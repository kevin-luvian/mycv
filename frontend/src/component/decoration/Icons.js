...
export const ColoredIcon = ({
  ...
  return (
    <Component
      className={concat(className, styles.icon, color)}
      onClick={onClick}
      {...attr}
    />
  );
...
export const TextIcon = ({ className, color, children, onClick, ...attr }) => {
  if (!color) color = iconColors.default;
  return (
    <div
      className={concat(className, styles.texticon, color)}
      onClick={onClick}
      {...attr}
    >
      {children}
