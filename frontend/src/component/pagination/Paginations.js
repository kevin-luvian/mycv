...
export const BasicPagination = ({ className, onChange, pageSize }) => {
  ...
  const findChunk = useCallback(
    (pageNum) => Math.max(Math.floor(pageNum / CHUNK_SIZE), 0),
    []
  );
  const firstChunkPage = useCallback((chunkNum) => chunkNum * CHUNK_SIZE, []);
  const lastChunkPage = useCallback(
    (chunkNum) => Math.min(firstChunkPage(chunkNum) + CHUNK_SIZE, pageSize),
    [firstChunkPage, pageSize]
  );
  ...
  const changePage = useCallback(
    (pageNum) => setActivePage(constraint(pageNum, 0, pageSize - 1)),
    [pageSize]
  );
  ...
  return (
    <div className={concat(className, styles.basicPagination)}>
      {findChunk(activePage) > 0 && (
        <div onClick={() => slideChunk(-1)} className={styles.pagenum}>
          {"<<"}
        </div>
      )}
      <div
      ...
      {pageNums.map((pageNum, index) => (
        <div
          className={concat(
            styles.pagenum,
            pageNum === activePage ? styles.active : ""
          )}
          key={index}
...
export const IndexedPagination = ({
  className,
  onChange,
  itemSize,
  perPage,
  ...
  const handleChange = useCallback(
    (page) => onChange?.(page * perPage, page * perPage + perPage),
    [onChange, perPage]
  );
