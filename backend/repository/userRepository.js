 ...
 */
const findById = id => {
    return User.findById(id, (err) => { if (err) debug("findById", err); });
}
...
const findByUsername = str => {
    return User.findOne({ username: str }, (err) => { if (err) debug("findByUsername", err); });
}
 ...
 */
const hashPass = pass => {
    return bcrypt.hashSync(pass, parseInt(process.env.SALT_ROUNDS));
}
